...
export class BarbersService {
   ...
   */
  async searchBarbers(filters: SearchFilters) {
    const {
      searchTerm,
      serviceIds,
      minPrice,
  ...
  async getBarberProfile(barberId: string) {
    ...
    if (barber.status !== 'ACTIVE') {
      throw new AppError('Barber profile is not available', 403);
    }
   ...
   */
  async getAvailability(barberId: string, query: AvailabilityQuery) {
    ...
    if (!service) {
      throw new AppError('Service not found for this barber', 404);
    }
  ...
  async getBarberReviews(barberId: string, page = 1, limit = 10) {
    ...
    const [reviews, total, stats] = await Promise.all([
      ...
      prisma.review.count({ where: { appointment: { barberId } } }),
   ...
   */
  private async getReviewStats(barberId: string) {
  ...
  private generateTimeSlots(
    ...
    while (currentTime < endDateTime) {
      ...
      const isBooked = bookedSlots.some((appointment: any) => {
